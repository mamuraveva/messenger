...
bool send_packet(int socket, const std::vector<unsigned char>& data) {
    ...
    if (!send_all(socket, reinterpret_cast<unsigned char*> (&network_size), sizeof(network_size))) {
        return false;
    }
    if (!data.empty()) {
...
bool recv_packet(int socket, std::vector<unsigned char>& data) {
    ...
    if (!recv_all(socket, reinterpret_cast<unsigned char*> (&network_size), sizeof(network_size))) {
        return false;
    }
