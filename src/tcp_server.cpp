...
bool TcpServer::validatePort(int port) { return port >= 1024 && port <= 65535; }
...
void TcpServer::accept_clients() {
  ...
  if (client_a < 0) {
    throw std::runtime_error(sock_error("Ошибка accept (первый клиент)"));
  }
  ...
  if (client_b < 0) {
    ::close(client_a);
    throw std::runtime_error(sock_error("Ошибка accept (второй клиент)"));
  }
  ...
  try {
    while (true) {
      ...
      if (!recv_packet(client_socket, data)) {
        std::cout << "Клиент отключился: socket = " << client_socket
                  << std::endl;
        break;
      }
    ...
    std::cerr << "Ошибка при обработке клиента socket " << client_socket << ": "
              << e.what() << std::endl;
  }
...
void TcpServer::broadcast_message(const std::vector<unsigned char> &data,
  ...
  for (int client : clients) {
    ...
    try {
      if (!send_packet(client, data)) {
        throw std::runtime_error("send_packet вернул false для socket " +
                                 std::to_string(client));
      }
    } catch (const std::exception &e) {
      std::cerr << "Ошибка отправки пакета: " << e.what() << std::endl;
    }
  }
}
