...
void TcpServer::accept_clients() {
  while (true) {
    ...
    if (client_a < 0) {
      std::cerr << sock_error("Ошибка accept (первый клиент пары)") << std::endl;
      continue;
    }
    ...
    if (client_b < 0) {
      std::cerr << sock_error("Ошибка accept (второй клиент пары)") << std::endl;
      ::close(client_a);
      continue;
    }
    ...
    try {
      ...
      std::cerr << "Ошибка при создании пары: " << e.what() << std::endl;
      ::close(client_a);
      ::close(client_b);
    }
  }
