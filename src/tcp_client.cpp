...
bool TcpClient::connect(const std::string& ip, int port) {
    ...
    if (::connect(socket, reinterpret_cast<sockaddr*>(&serverAddr), sizeof(serverAddr)) < 0) {
...
bool TcpClient::do_handshake() {
    ...
    if (role == 0x01) {
        ...
        std::vector<unsigned char> my_pub(crypto_.pub_key(), crypto_.pub_key() + crypto_kx_PUBLICKEYBYTES);
        if (!send_packet(socket, my_pub)) {
            std::cerr << "Ошибка отправки pub_key\n";
            return false;
        }
        std::vector<unsigned char> their_pub;
        if (!recv_packet(socket, their_pub) || their_pub.size() != crypto_kx_PUBLICKEYBYTES) {
